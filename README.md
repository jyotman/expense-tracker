...
8. **Widget** — a home-screen widget shows the current month's spend and a one-tap "add expense" (Jetpack Glance).
9. **Backup & restore** — export/import the full dataset as a JSON file via the system picker (save to Drive/Dropbox/local); OS auto-backup is the automatic safety net.
10. **On-device, no server** — all data and all parsing stay on the phone.
...
2. **Foreground — on-device LLM.** When AI is enabled and the model is available, the **ML Kit GenAI Prompt API** (Gemini Nano, via the system AICore) reads the raw notification in the prefilled form and extracts `{amount, merchant, category}` itself — including cross-checking the figures (e.g. picking the spent amount over the available balance). AI-filled fields are marked with a sparkle and clear that mark once you edit them.
